document.getElementById("submit_button").addEventListener("click", function(event){
  ...
  function login()
  {
    let user = document.getElementById("user").value, password = document.getElementById("password").value, user_validate = false;
  
    if(user=="" || user==null || user==" " || password=="" || password==null || password==" ")
    {
        mensaje("existen campos vacíos.");
    }else
    {
      bd_users.forEach((usuario) => {
        if(usuario[0]==user && usuario[1]==password){user_validate=true}
      });
  ...
  function add_usr()
  {
    let user = document.getElementById("user").value, password = document.getElementById("password").value;
  ...
  function change_pass()
  {
    ...
    {
        bd_users.forEach((usuario) => {
          if(usuario[0]==user_change){usuario[1] = document.getElementById("password").value}
